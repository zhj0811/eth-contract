...
contract HorseContract is ERC721Enumerable, Ownable {
    ...
    returns (uint16[5] memory gifts)
    {
        ...
        for (uint i =0; i <5; i++) {
            ...
            uint16 temp =uint16(_createRandom())%(gift+1);
            gifts[i] = temp;
            gift = gift-temp;
        }
        return gifts;
    ...
    function _mixGifts(uint16[5] storage _matron, uint16[5] storage _sire) internal returns(uint16[5] memory gifts) {
        //        for (uint i =0; i <5; i++){
    ...
    function _isReadyToBreed(Horse storage _horse) internal view returns (bool) {
        // In addition to checking the cooldownEndBlock, we also need to check to see if
        // the cat has a pending birth; there can be some period of time between the end
        // of the pregnacy timer and the birth event.
        return (_horse.generation == 0);
    }
    ...
    function _canCreateGen0Horse() internal view returns (bool)
    {
        uint256 index =((block.timestamp-deployTime)/WEEK_DURATION) + 1;
        //        require(index < 680);
        return (gen0CreatedCount < (index*10));
    }

    function createGen0Horse(address _owner) external onlyOwner returns(uint256) {
    ...
    returns(uint256)
    {
        ...
        require(ownerOf(_matronId)== msg.sender, "not matron owner");

        require(_isSiringPermitted(_sireId, _matronId), "sire not allowed");

        // Grab a reference to the potential matron
        Horse storage matron = horses[_matronId];

        require(!matron.sex, "matron not female");
        // Make sure matron isn't pregnant, or in the middle of a siring cooldown
        require(_isReadyToBreed(matron), "matron not first generation");

        // Grab a reference to the potential sire
        Horse storage sire = horses[_sireId];

        // Make sure sire isn't pregnant, or in the middle of a siring cooldown
        require(_isReadyToBreed(sire), "sire not first generation");
        require(sire.sex);
